...
//protocol YearInViewCellDelegate: AnyObject {
//	func 
//}

class YearInViewCell: UICollectionViewCell {
...
class YearInViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		self.collectionView.register(YearInViewCell.self, forCellWithReuseIdentifier: "YearInViewCell")
	}
	
	@objc func shareButtonTapped() {
		// Ref - https://stackoverflow.com/questions/18649920/uicollectionview-current-visible-cell-index
		guard let cell = self.collectionView.visibleCells.first as? YearInViewCell else { return }
		self.image = cell.scrollView.takeScrollViewScreenShot()
	}
}

extension YearInViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
	func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
		return 4
	}
	
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		return collectionView.dequeueReusableCell(withReuseIdentifier: "YearInViewCell", for: indexPath)
	}
}
